...
public class Pesquisador
{
    ...
    {
        List<Prof> lista = new List<Prof>();
        var AlunoDf = uni.Alunos;
        var ProfDf = uni.Professores;

        foreach (var prof in ProfDf)
        {
            Prof ObjectProf = new Prof();
            ObjectProf.nome = prof.Nome;
            ObjectProf.qtdAluno = 0;
            ...
            foreach (var aluno in AlunoDf)
            {
                ...
                for (int i = 0; i < len; i++)
                    if (prof.DepartamentoID == aluno.TurmasMatriculados[i])
                        ObjectProf.qtdAluno+=1;
            }
            lista.Add(ObjectProf);
        }

        var x = lista.OrderByDescending(j => j.qtdAluno);
        int Count = 0;
        foreach (var item in x)
        {
            Count+=1;
            Console.WriteLine($"{Count}° Professor: {item.nome} -- Alunos: {item.qtdAluno}");
        }
    }
